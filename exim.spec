...
%{__cp} src/EDITME Local/Makefile
%{__sed} -i 's@^# LOOKUP_MODULE_DIR=.*@LOOKUP_MODULE_DIR=%{_libdir}/exim/%{version}-%{release}/lookups@' Local/Makefile
...
%{__mkdir_p} %{buildroot}%{_sysconfdir}/exim

cd build-`scripts/os-type`-`scripts/arch-type`
%{__install} -m 4775 exim %{buildroot}%{_sbindir}
...
do
  %{__install} -m 0755 ${i} %{buildroot}%{_sbindir}
done

%{__mkdir_p} %{buildroot}%{_libdir}/exim/%{version}-%{release}/lookups
for i in mysql.so pgsql.so
do
  %{__install} -m 755 lookups/${i} \
  %{buildroot}%{_libdir}/exim/%{version}-%{release}/lookups
done

cd ..

%{__install} -m 0644 src/configure.default %{buildroot}%{_sysconfdir}/exim/exim.conf
%{__install} -m 0644 %SOURCE11 %{buildroot}%{_sysconfdir}/pam.d/exim

%{__mkdir_p} %{buildroot}/usr/lib
pushd %{buildroot}/usr/lib
%{__ln_s} -f ../sbin/exim sendmail.exim
popd

pushd %{buildroot}%{_sbindir}/
%{__ln_s} -f exim sendmail.exim
popd

pushd %{buildroot}%{_bindir}/
%{__ln_s} -f ../sbin/exim mailq.exim
%{__ln_s} -f ../sbin/exim runq.exim
%{__ln_s} -f ../sbin/exim rsmtp.exim
%{__ln_s} -f ../sbin/exim rmail.exim
%{__ln_s} -f ../sbin/exim newaliases.exim
popd

%{__install} -d -m 0750 %{buildroot}%{_var}/spool/exim
%{__install} -d -m 0750 %{buildroot}%{_var}/spool/exim/db
%{__install} -d -m 0750 %{buildroot}%{_var}/spool/exim/input
%{__install} -d -m 0750 %{buildroot}%{_var}/spool/exim/msglog
%{__install} -d -m 0750 %{buildroot}%{_var}/log/exim

%{__mkdir_p} %{buildroot}%{_mandir}/man8
%{__install} -m 644 doc/exim.8 %{buildroot}%{_mandir}/man8/exim.8
pod2man --center=EXIM --section=8 \
  %{buildroot}/usr/sbin/eximstats \
...
%{__install} -m 755 %{SOURCE25} %{buildroot}%{_libexecdir}

%if %{with clamav}
%{__install} -m 644 %{SOURCE26} %{buildroot}%{_unitdir}
%endif

%{__mkdir_p} %{buildroot}%{_sysconfdir}/logrotate.d
%{__install} -m 0644 %SOURCE4 %{buildroot}%{_sysconfdir}/logrotate.d/exim
...
gzip < /dev/null > %{buildroot}%{_mandir}/man1/mailq.1.gz
...
%{__cat} <<EOF > %{buildroot}%{_sysconfdir}/sysconfig/clamd.exim
CLAMD_CONFIG='%_sysconfdir/clamd.d/exim.conf'
CLAMD_SOCKET=%{_var}/run/clamd.exim/clamd.sock
EOF
%{__ln_s} -f clamd %{buildroot}/usr/sbin/clamd.exim

%{__mkdir_p} %{buildroot}%{_tmpfilesdir}
%{__install} -m 0644 %{SOURCE12} %{buildroot}%{_tmpfilesdir}/exim-clamav.conf
%{__mkdir_p} %{buildroot}%{_var}/run/clamd.exim
%{__mkdir_p} %{buildroot}%{_var}/log
...
%{__mkdir_p} %{buildroot}/%_sysconfdir/cron.daily
%{__install} -m 755 %{SOURCE22} %{buildroot}/%_sysconfdir/cron.daily/greylist-tidy.sh
%{__install} -m 644 %{SOURCE23} %{buildroot}/%_sysconfdir/exim/trusted-configs
touch %{buildroot}/%_var/spool/exim/db/greylist.db
...
%pre
%{_sbindir}/groupadd -g 93 %{group} 2>/dev/null
%{_sbindir}/useradd -d %{_var}/spool/exim -s /sbin/nologin -G mail -M -r -u 93 -g %{group} %{user} 2>/dev/null
...
if [[ ! -f /etc/pki/tls/certs/exim.pem -a -f %{_datadir}/ssl/certs/exim.pem ]]; then
  %{__cp} %{_datadir}/ssl/certs/exim.pem /etc/pki/tls/certs/exim.pem
  %{__cp} %{_datadir}/ssl/private/exim.pem /etc/pki/tls/private/exim.pem
fi
...
if [[ ! -r %{_var}/spool/exim/db/greylist.db ]]; then
  ...
  %{__chown} %{user}.%{group} %{_var}/spool/exim/db/greylist.db
  %{__chmod} 0660 %{_var}/spool/exim/db/greylist.db
fi
...
- add rm -rf %{buildroot} in install section
- use %%{_tmppath}
...
- removed Vendor header since it is going to be marked Red Hat in our build system.
- quiet setups
- strip binaries
- fixed so that man pages can be auto gzipped by new RPM (in files list /usr/man/*/* )
...
- Changed the default configuration file to make exim run as user and group 'mail'.
...
* Thu Jul 08 1999 Mark Bergsma <mark@mbergsma.demon.nl>
- Added procmail transport and director, and made that the default.
...
- The exim logfiles will now only be removed when uninstalling, not upgrading.
